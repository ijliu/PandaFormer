...
def genGaussionMap(img,x,y,p=16):
    H,W = img.shape
    ...
    for h in range(H):
        for w in range(W):
            hy = h//p
            wx = w//p
            img[h][w] = np.exp(-((hy-y)**2 + (wx-x)**2)/(2*16**2))
    return img
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels,times, pos_tmp, _, meta) in enumerate(train_loader):
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, times,pos_tmp, _, meta) in enumerate(val_loader):
        #################################
        B,C,T,H,W = inputs.shape
