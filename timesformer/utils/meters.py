...
class TestMeter(object):
    ...
    def update_stats(self, preds, labels, clip_ids):
        ...
        for ind in range(preds.shape[0]):
            ...
            # with open(f"preds_{self.ensemble_method}.txt", "a+") as fp:
            #     print(self.video_preds[vid_id], file=fp)
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            num_correct_per_classes, num_per_classes = metrics.per_class_correct(
                self.video_preds, self.video_labels
            )
            
            mean_classes_accuracy = num_correct_per_classes / num_per_classes
            
            mean_class_str = ""
            for k,value in enumerate(mean_classes_accuracy):
                mean_class_str += f"{k:2} : {value:.6f}"
            self.stats["mean_class_accuracy"] = mean_class_str
            # mean_classes_accuracy
