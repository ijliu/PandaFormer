...
def per_class_correct(preds, labels):
    assert preds.size(0) == labels.size(
        0
    ), "Batch dim of predictions and labels must match"
    _top_max_k_vals, top_max_k_inds = torch.topk(
    ...
    # (i, j) = 1 if top i-th prediction for the j-th sample is correct.
    top_max_k_correct = top_max_k_inds.eq(rep_max_k_labels)
    classes, num_class_pre_class = torch.unique(labels, return_counts=True)
    num_correct_per_classes = []
    for cls in classes:
        tmp = torch.where(rep_max_k_labels == cls)
        class_correct = top_max_k_correct[tmp]
        num_correct_per_classes.append([class_correct.float().sum()])
    
    num_correct_per_classes =  torch.tensor(np.array(num_correct_per_classes)).view(-1)
    return num_correct_per_classes, num_class_pre_class
